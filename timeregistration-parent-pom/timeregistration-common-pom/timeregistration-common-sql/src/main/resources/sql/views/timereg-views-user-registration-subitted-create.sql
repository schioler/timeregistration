create or replace view vw_user_registration_submitted as
select 
    ...
    event.id as event_id, event.amount, event.comments
from user u, registration_item item, vw_client_code cc, vw_provider_code pc, registration_submitted event
where 
    u.id = event.user_id  
